/**
 * 打印
 * https://github.com/rowanwins/leaflet-easyPrint/blob/gh-pages/dist/leaflet.easyPrint.js
 */
...
/*增加*/
function addClickListenerToPrint(options) {
    addCSS();
    if (options.elementsToHide) {
        var htmlElementsToHide = document.querySelectorAll(options.elementsToHide);

        for (var i = 0; i < htmlElementsToHide.length; i++) {
            htmlElementsToHide[i].className = htmlElementsToHide[i].className + ' _epHidden';
        }
    }
    options.map.fire("beforePrint");
    window.print();
    options.map.fire("afterPrint");
    if (options.elementsToHide) {
        var htmlElementsToHide = document.querySelectorAll(options.elementsToHide);
        for (var i = 0; i < htmlElementsToHide.length; i++) {
            htmlElementsToHide[i].className = htmlElementsToHide[i].className.replace(' _epHidden', '');
...
export { easyPrint, addClickListenerToPrint };
